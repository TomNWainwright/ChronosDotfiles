...
var common = (function() {
	...
	function createNaClModule(name, tool, path, width, height, attrs) {
		...
		// event fires.
		var listenerDiv = window.globalAppBody.querySelector('#listener');
	...
	function handleError(event) {
		...
		// loaded.
		var moduleEl = window.globalAppBody.querySelector('#nacl_module');
	...
	function moduleDidLoad() {
		common.naclModule = window.globalAppBody.querySelector('#nacl_module');
	...
	function logDebug(message) {
		log.debug("[nacl]", message);
	}
	function logError(message) {
		log.error("[nacl]", message);
	}

	function echoBack(msg) {
		log.debug("echo back from nacl: ", msg);
	}
	function echo(msg) {
		log.debug("send echo to nacl: ", msg);
		naclPostMessage("echo", msg);
	}	
	function echoWithCallback(msg) {
		log.debug("send echo to nacl: ", msg);
		naclPostMessage("echoWithCallback", msg, function(recvMsg) {
			log.debug("echo back from nacl callback: ", recvMsg);
		});
	}	

	/** receive message from nacl
	 */
	var funMap = {
	...
	function registerFun(name, fun) {
		funMap[name] = fun;
	}

	var waitingCallbacks = {};
	var waitingCbNextId = 0;
	function naclPostMessage(name, msg, cb) {
		...
		common.naclModule.postMessage({name: name, message: msg, callbackId: id});
	};

	// msg: { id, keep[option], arguments[option] }
	function handleCallback(msg) {
		var cb = waitingCallbacks[msg.id];
		if (cb) {
			if (!msg.keep)
				delete waitingCallbacks[msg.id];
			if (msg.arguments instanceof Array)
				cb.apply(window, msg.arguments);
			else
				cb(msg.arguments);
		}
		else
			log.warn("callback from nacl no handle found, id: ", msg.id);
	}
	 ...
	 */
	function handleMessage(naclMsg) {
		var evt = naclMsg.data;
		if (typeof evt === "string") {
			// stdout by NACL_EXE_STDOUT=DEBUG_ONLY:dev://postmessage...
			log.debug("[stdout]", evt);
		}
		else if (typeof evt !== "object") {
			log.error("error type of nacl message: " , evt);
		}
		else {
			var fun = funMap[evt.name];
			if (!fun)
				log.warn("nacl: no handle for message: ", evt.name);
			else {
				fun(evt.message);
			}
		}
	...
	function updateStatus(opt_message) {
		...
		var statusField = window.globalAppBody.querySelector('#statusField');
		if (statusField) {
			statusField.innerHTML = statusText;
		}
		log.debug("NACL Statue: " + statusText);
	}
	...
	return {
		...
		echo: echo,
		echoWithCallback: echoWithCallback,
		naclPostMessage: naclPostMessage,
	};
...
function load() {
	...
	if (loadFunction) {
		...
		var path;
		if (!searchVars.config) {
			chrome.runtime.getPlatformInfo(function(nacl) {
				path = '_platform_specific/' + nacl.nacl_arch;
				...
				window.nacl = nacl; // for crash report.
			});
		}
		else {
