            ...
            try {
                ...
                xhr.onreadystatechange = function () {
                    if (xhr.readyState == 4) {
                        ...
                        var total_amount = getXPathString(doc, "//tr[@class='os-row']//td[@class='os-price-col' and contains(.,'tax')]/span");
                        var tax_amount = getXPathString(doc, "//tr[@class='os-row']//td[@class='os-price-col' and contains(.,'tax')]/div");

                        var tax_location = getXPathString(doc, "substring-after(//div[@class='location-change']/div/text(), 'location:')") || '';
                ...
                xhr.open("GET", "https://www.google.com/shopping/product/14411758030637192837", true);
                xhr.send();
            } catch (e) {
...
function onRequest(request, sender, callback) {
    ...
    function gateway() {
        ...
        allStorageGet(function (local) {
            if (request.url && !isSuppressed(request.url, local.suppressedRetailers)) {
    ...
    function action() {
        ...
        if (request.action == 'getRetailer') {
            ...
            if (wishlist_url != null && current_retailer) {
            ...
            var url = "https://tb.priceblink.com/coupons_response_array.php?uid=" + uid + "&browser=" + currentBrowserType + "&ver=" + ver + "&rid=" + current_retailer.id + "&rids=" + retailer_ids + "&partnerid=0";
...
function getCoupons(callback) {
    ...
    if (lastGetCouponsCall && now() - lastGetCouponsCall < interval && coupons.length > 0) {
        console.warn('Get coupons call too frequent. Cancelled');
        usePreviousResults = true;
    }
    lastGetCouponsCall = now();
    setTimeout(function () {
      lastGetCouponsCall = 0;
    }, interval);

    if (usePreviousResults) {
      allStorageGet(function (local) {
        callback(prepareCallbackParams(coupons, local.disabled_retailers));
      });
      return;
    }
            ...
            allStorageGet(function (local) {
                ...
                browser.tabs.query({active: true}, function (tabs) {
                    ...
                    browser.tabs.sendMessage(tab.id, {
                        ...
                        var url = "https://tb.priceblink.com/universal_scrapes.php?uid=" + uid + "&browser=" + currentBrowserType + "&ver=" + ver + "&rid=" + response.retailer_id + "&partnerid=0";
                        var xhr = new XMLHttpRequest();
                        xhr.onreadystatechange = function () {
                            if (xhr.readyState == 4) {
    ...
    xhr.open("GET", "https://tb.priceblink.com/coupons?rid=" + current_retailer.id + "&uid=" + uid + "&browser=" + currentBrowserType + "&ver=" + ver + "&partnerid=0&code=0", true);
    xhr.send();
}
 ...
 */
function getAmazonSellerInfo(params, scraped_retailers_arr, callback, method) {
    ...
    xhr.onreadystatechange = function () {
        ...
        if (xhr.readyState == 4) {
            ...
            postScrapedData(params, scraped_retailers_arr, callback, method);
...
function getAmazon(params, scraped_retailers_arr, callback, method) {
...
// 1 step scrape
function getGTIN(params, callback) {
    allStorageGet(function (local) {
        var url = (local.country === 'uk') ?
            "https://www.google.co.uk/" : "https://www.google.com/";
            ...
            if (xhr.readyState == 4) {
                ...
                if (parseInt(no_results)) {
                  ...
                  if (params.asin != undefined && params.asin != "") {
                    ...
                    getAmazonSellerInfo(params, [], callback, "Google 1 Step Scrape");

                  } else {

                    params.am_url ? getAmazon(params, [], callback, "Google 1 Step Scrape") : basicScrape(params, callback);
                ...
                if (href.length == 0) {
                    ...
                    if (params.asin != undefined && params.asin != "") {
                        ...
                        getAmazonSellerInfo(params, scraped_retailers_arr, callback, "Google 1 Step Scrape");

                    } else if (params.am_url != "") {

                        getAmazon(params, scraped_retailers_arr, callback, "Google 1 Step Scrape");

                    } else {

                        postScrapedData(params, scraped_retailers_arr, callback, "Google 1 Step Scrape");
...
// 2 step scrape
function getPrices(params, id, callback) {
    allStorageGet(function (local) {
        var url = (local.country === 'uk') ?
            "https://www.google.co.uk/" : "https://www.google.com/";

        url += "shopping/product/" + id + "/online?sa=X&prds=scoring:tp";
        ...
        xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                ...
                if (params.asin != undefined && params.asin != "") {
                    ...
                    getAmazonSellerInfo(params, scraped_retailers_arr, callback, "Google 2 Step Scrape");

                } else if (params.am_url != "") {

                    getAmazon(params, scraped_retailers_arr, callback, "Google 2 Step Scrape");

                } else {

                    postScrapedData(params, scraped_retailers_arr, callback, "Google 2 Step Scrape");
...
function basicScrape(params, callback) {
    postScrapedData(params, "", callback, "No Scraped Data");
}

function postScrapedData(params, data, callback, method) {
    ...
    xhr.send("retailers=" + encodeURIComponent(JSON.stringify(data)).replace(/'/g, "%27").replace(/"/g, "%22") + "&token=" + params.token + "&tax=1");
}
...
function enableCouponsForRetailer() {
    browser.tabs.query({active: true}, function (tabs) {
        ...
        restoreRetailer(current_retailer.id);
...
browser.tabs.onActivated.addListener(function (tab) {
    ...
    browser.tabs.get(tab.tabId, function (tab) {
        current_retailer = getRetailer(tab.url);
    });
