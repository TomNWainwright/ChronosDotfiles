...
var Proto = _.extend(API.prototype, Backbone.Events);
...
Proto.post = function(url, data, cb) {
  ...
  if (arguments.length == 4) {
    cb = arguments[3];
    post.set('Authorization', 'Basic ' + this.getAuthPayload());
  }
  
  post.end(function(res) {
    if (res.status === 0) {
      return cb(new Error('connection_problems'), res);
    } else if (res.status !== 200) {
      return cb(new Error(errorify(res)), res);
    }
    cb(null, res);
  });
};
...
Proto.sync = function(clipboard) {
  ...
  if (!cryptoKey) {
    _gaq.push(['_trackEvent', 'sync', 'outgoing_failed_no_crypto_key']);
  ...
  this.post('/sync', data, true, function(err, res) {
    DEBUG('superagent sync status ' + res.status);
    if (err) {
      _gaq.push(['_trackEvent', 'sync', 'outgoing_failed_server']);
      self.trigger('err', err);
    }
  });

  DEBUG('superagent sync start ' + JSON.stringify(data));
  _gaq.push(['_trackEvent', 'sync', 'outgoing']);
};
