...
(function (factory) {
  ...
  return function (global, window, document, undefined) {
    ...
    var Type = {
      ...
      'isEmptyObject': function isEmptyObject(variable) {
        for (var name in variable) {
          if (variable.hasOwnProperty(name)) return false;
        }
        ...
        'fixColors': function fixColors(str) {
          return str.replace(/(rgba?\(\s*)?(\b[a-z]+\b)/g, function ($0, $1, $2) {
            if (CSS.Lists.colorNames.hasOwnProperty($2)) {
              return ($1 || 'rgba(') + CSS.Lists.colorNames[$2] + ($1 ? '' : ',1)');
            }
            return $1 + $2;
          });
        },
              ...
              CSS.Normalizations.registered[colorName] = function (type, element, propertyValue) {
                switch (type) {
                  ...
                  case 'inject':
                    ...
                    if (/^rgb/.test(propertyValue)) return propertyValue;
    ...
    var animate = function animate() {
      ...
      var elementsWrapped, argumentIndex;
...
},{"promise-polyfill":1}],7:[function(require,module,exports){
'use strict';
...
var _velocity = require('plugins/velocity');

var _velocity2 = _interopRequireDefault(_velocity);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

window.Velocity = _velocity2.default;
...
(function () {
  ...
  window.showSpeedPromo = function () {
    ...
    return mode === 'proxy' || Boolean(_.find(filters, function (_ref) {
  ...
  /** @type {(string|null)} */
  var trialPremiumToken = storage.getItem('trial_premium_token');
  ...
  if (congratsTabId) {
    tabOpenPromise = Browser.tabs.query().then(function (tabs) {
      return Boolean(_.find(tabs, function (_ref2) {
        var id = _ref2.id;
  ...
  (function () {
    var condition = !ShowedOffers.includes('apps page') && !trialPremiumToken && storage.getItem('installBrowsecOnMobile') === 'ready';
    if (!condition) return;

    tabOpenPromise.then(function (tabOpen) {
      // Congratulations tab open
      if (tabOpen) {
  ...
  /** In case of trial token show premium token invitation (just once) */
  if (!ShowedOffers.includes('trial premium') && trialPremiumToken) {
    tabOpenPromise.then(function (tabOpen) {
      // Congratulations tab open
      if (tabOpen) {
        Browser.tabs.update(congratsTabId, { 'url': 'offer_v2.html' });
      } else {
        Browser.tabs.create({ 'active': false, 'url': 'offer_v2.html' });
      }

      ShowedOffers.push('trial premium');
      storage.removeItem('congrats_tab_id');
    });
  }
  ...
  window.addEventListener('WebComponentsReady', function () {
    ...
    if (store.getState().pac.mode === 'broken') {
      elements.push('popup-proxy-blocked');
    }
...
},{"plugins/velocity":2,"polyfills/Element.append":3,"polyfills/Element.matches":4,"polyfills/Element.prepend":5,"polyfills/Promise":6}]},{},[7]);
